...
log_scale_breaks <- c(-9.210340, -6.907755, -4.605170, -2.302585, 0.000000, 2.302585, 4.605170)
...
ggplot(mod1s_pred, aes(log(x), predicted)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), fill = "forestgreen", alpha = 0.1) +
  geom_point(data = alien_dat, aes(log(scale), slope), pch = 20, col = "gray22", size = 3) +
  ...
  theme(axis.text.x = element_text(size = 8), axis.text.y = element_text(size = 8),
        axis.title = element_text(size = 11), legend.text = element_text(size = 10)) +
  scale_x_continuous(breaks = log_scale_breaks[-1], labels = log_scale_labels[-1])

# Correlation analyses
cor(alien_dat$slope, alien_dat$scale, use = "pairwise.complete.obs", method = "pearson")
...
ggplot(alien_dat, aes(y = log(scale), x = r2m)) +
  ...
  geom_point(position = position_dodge(width = dodge_width), size = 3, fill = "forestgreen", pch = 23) +
  ...
  scale_y_continuous(breaks = log_scale_breaks[-1], labels = log_scale_labels[-1])
...
Anova(mod2.2)
summary(mod2.2)
par(mfrow = c(2, 2)); plot(mod2.2); par(mfrow = c(1, 1))

plot_model(mod2, type = "pred", terms = c("scale"), show.data = TRUE)
mod1s_pred <- get_model_data(mod2, type = "pred", terms = "scale[0.001:100, by=0.001]")

ggplot(mod1s_pred, aes(log(x), predicted)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), fill = "#5F9EA0", alpha = 0.1) +
  geom_point(data = archaeophyte_dat, aes(log(scale), slope), pch = 20, col = "gray22", size = 3) +
...
cor(archaeophyte_dat$slope, archaeophyte_dat$scale, use = "pairwise.complete.obs", method = "pearson")
Hmisc::rcorr(log(archaeophyte_dat$slope + 10), log(archaeophyte_dat$scale), type = "spearman")
Hmisc::rcorr(log(archaeophyte_dat$slope + 10), log(archaeophyte_dat$scale), type = "pearson")

# R2 plot for archaeophytes
ggplot(archaeophyte_dat, aes(y = log(scale), x = r2m)) +
  geom_vline(xintercept = 0, color = "gray33", linetype = "dashed") +
  geom_point(position = position_dodge(width = dodge_width), size = 3, fill = "#5F9EA0", pch = 23) +
  geom_errorbarh(aes(xmin = 0, xmax = r2m), col = "#5F9EA0", linetype = "dashed",
                 position = position_dodge(width = dodge_width), height = 0.1) +
  theme(legend.key = element_blank(), axis.text.x = element_text(size = 8),
        axis.text.y = element_text(size = 8),
        axis.title = element_text(size = 11, face = "bold"),
...
mod3s_pred <- get_model_data(mod3.1, type = "pred", terms = "scale[0.1:100, by=0.001]")

ggplot(mod3s_pred, aes(log(x), predicted)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), fill = "slateblue", alpha = 0.1) +
  geom_point(data = neoph_dat, aes(log(scale), slope), pch = 20, col = "gray22", size = 3) +
  ...
  scale_x_continuous(
    limits = c(-6.907755, 5),
    log_scale_breaks[-1], labels = log_scale_labels[-1])

cor(neoph_dat$slope, neoph_dat$scale, use = "pairwise.complete.obs", method = "pearson")

# R2 plot for neophyte species
ggplot(neoph_dat, aes(y = log(scale), x = r2m)) +
  geom_vline(xintercept = 0, color = "gray33", linetype = "dashed") +
  geom_point(position = position_dodge(width = dodge_width), size = 3, fill = "slateblue", pch = 23) +
...
mod2s_pred <- get_model_data(mod4.2, type = "pred", terms = "scale[0.1:100, by=0.001]")

ggplot(mod2s_pred, aes(log(x), predicted)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), fill = "brown", alpha = 0.1) +
  geom_point(data = invas_dat, aes(log(scale), slope), pch = 20, col = "gray22", size = 3) +
  ...
  scale_x_continuous(
    limits = c(-6.907755, 5),
    breaks = log_scale_breaks[-1], labels = log_scale_labels[-1])


cor(invas_dat$slope, invas_dat$scale, use = "pairwise.complete.obs", method = "pearson")

# R2 plot for invasive species
ggplot(invas_dat, aes(y = log(scale), x = r2m)) +
  geom_vline(xintercept = 0, color = "gray33", linetype = "dashed") +
  geom_point(position = position_dodge(width = dodge_width), size = 3, fill = "brown", pch = 23) +
