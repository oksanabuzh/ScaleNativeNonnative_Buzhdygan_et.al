...
results <- read_csv("data/model_results_summary.csv")%>% 
  ...
  filter(model_id%in% c("builtup_250m", "climate") & 
...
Suppl_dat2 <- results %>% 
  filter(!scale==0.0001) %>% 
filter(response_var %in% c("non_native_percent", "archaeophyte_percent") |
       response_var %in% c("invasive_percent", "neophyte_percent") & !scale %in% c(0.0001, 0.001, 0.01))%>%
  mutate(response_variable=case_when(response_var=="non_native_percent" ~ "alien",
    ...
    dplyr::select(scale, response_variable, predictor, slope,	chisq,	
         p_value_chisq,	r2_partial,	r2m,	r2c) %>% 
  mutate(predictor=
...
results <- results %>% mutate(variable_new=
  ...
  mutate(variable_new =fct_relevel(variable_new, 
                                   ...
                                   "Croplands (250 m)", "Urban built-up (250 m)",
                                   "Croplands (500 m)", "Urban built-up (500 m)",
                                   "Croplands (1000 m)", "Urban built-up (1000 m)",
...
P1 <- ggplot(results %>% 
  ...
  theme(axis.text.x = element_text(size = 6, colour = "black"), 
        ...
        strip.background=element_rect(colour="black",
                                      fill="#EFF7EF")) +
  facet_wrap(~reorder(variable_new, desc(variable_new)), scales = "free", 
             ncol = 2) +
...
P2 <- ggplot(results %>% 
  ...
  theme(axis.text.x = element_text(size = 6, colour = "black"), 
        ...
        strip.background=element_rect(colour="black",
                                      fill="#F9F0F0")) +
  facet_wrap(~reorder(variable_new, desc(variable_new)), 
             scales = "free", ncol = 2) +
...
P4 <- ggplot(results %>% 
  ...
  mapply(function(x, z, signif) 
    geom_smooth(method="lm", 
                data=function(d) subset(d, predictor==z), 
                formula = x, 
                fill="#5F9EA0", col="#5F9EA0",  alpha=0.07,
                linetype = ifelse(signif == "Sign"| signif == "Marg",
                                  "solid", "dashed"),
                linewidth = case_when(
                  signif == "Sign" ~ 1.1,
