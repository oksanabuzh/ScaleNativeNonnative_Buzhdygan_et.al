...
sp2 <- sp |>
  mutate(
    ...
    p_a_archaeophyte = ifelse(archaeophyte > 0, 1, 0)
  )
...
alien_dat %>% 
  filter(type == "p_a") %>% 
  filter(!scale==0.0001) %>% 
  ggplot(aes(log(scale), archaeophyte_percent)) +
  geom_point(col="#5F9EA0", size=0.7, alpha=0.5,
             position = position_jitter(height = 0, width = 0.5)) +
  # scale_x_log10() +
  stat_summary(fun.data = mean_se, geom='errorbar', width=0.5, color='black')+
  stat_summary(fun.y = mean, geom="point", size=2, , color='black') + 
  stat_summary(geom = "line", fun = mean, col="black") +
  labs(y="Proportion of archaeophytes", 
       x=expression(paste('Grain size, ', m^{2}))) +
  # guides(shape = "none") +
  theme(axis.text.x = element_text(size=8), 
        axis.text.y = element_text(size=8),
...
alien_dat %>% 
  ...
  # stat_summary(fun.data = mean_sdl, col="blue") +
  # SE
stat_summary(fun.data = mean_se, geom='errorbar', width=0.5, color='#5F9EA0')+
  stat_summary(fun.y = mean, geom="point", size=2, , color='#5F9EA0') + 
  stat_summary(geom = "line", fun = mean, col="#5F9EA0") +
  # geom_point(col="forestgreen", size=2, alpha=0.6) +
  # scale_x_log10() +
  labs(y="Proportion of archaeophytes", 
       x=expression(paste('Grain size, ', m^{2}))) +
...
### native SR ----
mod1s<- glmer(archaeophyte_percent ~ 
                pca1_clima +
                native +
                (1|dataset),
              weights = total_species,
...
ggplot(mod1s_pred, aes(x, predicted)) +
  ...
  geom_point(data=alien_dat %>%  filter(type == "p_a" &  scale == 100) %>% 
               filter(!archaeophyte_percent==0), 
             aes(native, archaeophyte_percent),
             col="#5F9EA0", size=2, alpha=0.6) +
  labs( y ="Proportion of archaeophytes", x='Native species richness (SR)') +
  theme(axis.text.x = element_text(size=8), 
        axis.text.y = element_text(size=8),
        axis.title=element_text(size=11), 
        legend.text = element_text(size=10)) +
...
alien_dat_native_cover <- alien_dat %>%  filter(type == "p_a"&  scale == 100) %>% 
  dplyr::select (dataset, series, pca1_clima, archaeophyte_percent, 
                 invasive_percent, neophyte_percent, total_species) %>% 
  left_join(native_cover, by=c("series"))


mod2s<- glmer(archaeophyte_percent ~ 
                pca1_clima +
                native +
                (1|dataset),
              weights = total_species,
...
ggplot(mod2s_pred, aes(x, predicted)) +
  ...
  geom_point(data=alien_dat_native_cover %>% 
             ...
             aes(native, archaeophyte_percent),
             col="#5F9EA0", size=2, alpha=0.6) +
  labs(y="Proportion of archaeophytes", x='Cover of native species') +
  # size='Climate gradient (PC)') +
  # guides(shape = "none") +
  theme_bw()+
  geom_line(linetype=1, linewidth=1) +
