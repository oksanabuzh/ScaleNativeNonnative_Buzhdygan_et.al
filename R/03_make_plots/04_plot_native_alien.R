...
alien_dat <- left_join(sp2, climate_pc, by = "series") %>%
...
native_cover <- alien_dat %>%
  filter(type == "cover" & scale == 100) %>%
  dplyr::select(series, native)


alien_dat_native_cover <- alien_dat %>%
  filter(type == "p_a" & scale == 100) %>%
  dplyr::select(
    dataset,
    series,
...
ggplot(mod1s_neoph_pred, aes(x, predicted)) +
  ...
  labs(y = "Proportion of neophytes", x = 'Native species richness (SR)') +
  theme(
    axis.text.x = element_text(size = 8),
    axis.text.y = element_text(size = 8),
    axis.title = element_text(size = 11),
...
mod2s_neoph_pred <- get_model_data(
  mod2s_neoph,
  type = "pred",
  terms = "native[2:155, by=0.001]"
)
