...
alien_dat <- read_csv("data/model_results_summary.csv") %>%
  ...
  filter(model_id == "climate" | model_id == "disturbance" |
    model_id == "climate_native" & predictor == "native") %>%
  #  filter(response_var=="non_native_percent") %>%
  filter(!scale == 0.0001) %>%
  select(model_id, response_var, scale, predictor, r2_partial) %>%
  mutate(variable_new =
...
aes(x = as.factor(scale),
  ...
  #  scale_color_manual(values = c("#FDE725", "#8FD644", "#3ABA76", "#2A768E", "#443A83", "#450559"))+
  #  labs(x = NULL, y = NULL) +
...
# separately for invasive and alien ----
