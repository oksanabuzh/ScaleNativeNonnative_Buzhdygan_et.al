...
Table <- bind_rows(alien_select, invasive_select, neophyte_select, archaeophyte_select) %>% 
  mutate(variable_new=
...
Table2 <- bind_rows(alien_select, invasive_select, neophyte_select, archaeophyte_select) %>% 
  mutate(predictor=
           fct_recode(predictor,
                      "Climate PC"= "pca1_clima", 
                      "Soil pH"= "pH", 
...
write_csv(Table2, "results/Supplem_Data3_Model_selection_Table_scale-depandancy.csv")
