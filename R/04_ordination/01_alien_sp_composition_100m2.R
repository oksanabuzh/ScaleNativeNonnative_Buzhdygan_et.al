...
species.scores2 <- scores(nmds2, display = 'sp') %>%
  ...
  mutate(
    short_name = str_c(
      str_split_i(species, '\\s', 1) %>% str_sub(., 1, 4), # make short species names Genus species -> Gen.spe
      str_split_i(species, '\\s', 2) %>% str_sub(., 1, 4),
      sep = '.'
    )
  ) %>%
  left_join(
    traits_st %>%
      mutate(species = taxon_tpl) %>%
...
species.scores2$naturalisation_level <- as.factor(
  species.scores2$naturalisation_level
)
...
p2 <- ggplot(data = species.scores2, aes(x = NMDS1, y = NMDS2)) +
  ...
  geom_text_repel(
    data = species.scores2,
    aes(
      x = NMDS1,
      y = NMDS2,
      label = short_name,
      colour = naturalisation_level
    ),
    max.overlaps = 100
  ) +
...
coord_cont_standrd2 <- coord_cont2 %>%
  mutate(
    stand.NMDS1 = stand.NMDS1 * ordiArrowMul(fit2, rescale = TRUE, fill = 0.25)
  ) %>%
  mutate(
    stand.NMDS2 = stand.NMDS2 * ordiArrowMul(fit2, rescale = TRUE, fill = 0.25)
  ) %>%
  filter(
    Variables_new %in%
      c(
...
ggplot(data = species.scores2, aes(x = NMDS1, y = NMDS2)) +
  ...
  geom_text(
    data = coord_cont_standrd2,
    aes(
      x = stand.NMDS1 + 0.001 * sign(stand.NMDS1),
