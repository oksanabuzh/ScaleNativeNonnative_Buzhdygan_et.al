...
plot_proportion <- sp_data %>%  
  ...
  left_join(sp_data |> filter(type == "p_a") %>% 
            ...
            by=c("scale")) %>% 
  mutate(percent_plots_alien=round(non_native_plot_n*100/Plot_number,2))%>% 
  left_join(sp_data |> filter(type == "p_a") %>% 
              filter(invasive>0) %>% 
              count(scale, name = "invasive_plot_n"),
            by=c("scale")) %>% 
  mutate(percent_plots_invasive=round(invasive_plot_n*100/Plot_number, 2))%>% 
  left_join(sp_data |> filter(type == "p_a") %>% 
              filter(archaeophyte>0) %>% 
              count(scale, name = "archaeophyte_plot_n"),
            by=c("scale")) %>% 
  mutate(percent_plots_archaeophyte=round(archaeophyte_plot_n*100/Plot_number,2)) %>% 
  left_join(sp_data |> filter(type == "p_a") %>% 
              filter(neophyte>0) %>% 
              count(scale, name = "neophyte_plot_n"),
            by=c("scale")) %>% 
  mutate(percent_plots_neophyte=round(neophyte_plot_n*100/Plot_number, 2)) %>% 
...
plot_proportion_habitatas <- alien_data %>%  
  ...
  count(scale, name = "Plot_number") %>% 
  left_join(alien_data |> filter(type == "p_a") %>% 
              filter(non_native>0) %>% 
...
Table_mod3h <- emmeans(mod3h, list(pairwise ~ habitat_broad))$`pairwise differences of habitat_broad` %>% 
  as.tibble()
Table_mod3h

Table_mod3h_means <- emmeans(mod3h, list(pairwise ~ habitat_broad))$`emmeans of habitat_broad` %>% 
  as.tibble()
Table_mod3h_means


emmeans_m3_habitat <- cld(emmeans(mod3h, list(pairwise ~ habitat_broad)), 
                          Letters = letters) %>% arrange(habitat_broad)
emmeans_m3_habitat
...
alien_data_100 %>%  
  ...
  scale_color_manual(values = col)+
  scale_fill_manual(values = col2)+
  labs(y="Proportion of invasive species", x='Grassland type', 
       col="Grassland type",
       fill="Grassland type") +
  # guides(shape = "none") +
  theme_bw()+
  geom_text(data=emmeans_m3_habitat,
            aes(x=habitat_broad, y=c(0.055, 0.05, 0.11, 0.03, 0.07, 0.01, 0.01),
...
Table_mod4h <- emmeans(mod4h, list(pairwise ~ habitat_broad))$`pairwise differences of habitat_broad` %>% 
  as.tibble()
Table_mod4h

Table_mod4h_means <- emmeans(mod4h, list(pairwise ~ habitat_broad))$`emmeans of habitat_broad` %>% 
  as.tibble()
Table_mod4h_means


emmeans_m4_habitat <- cld(emmeans(mod4h, list(pairwise ~ habitat_broad)), 
                          Letters = letters) %>% arrange(habitat_broad)
emmeans_m4_habitat
...
alien_data_100 %>%  
  ...
  scale_color_manual(values = col)+
  scale_fill_manual(values = col2)+
  labs(y="Proportion of neophytes", x='Grassland type', 
       col="Grassland type",
       fill="Grassland type") +
  # guides(shape = "none") +
  theme_bw()+
  geom_text(data=emmeans_m4_habitat,
            aes(x=habitat_broad, y=c(0.06, 0.06, 0.11, 0.04, 0.07, 0.01, 0.01),
                label=emmeans_m4_habitat$.group),vjust=0.5, hjust=0.5, 
            size=4, col="black" , position=position_dodge(0))+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1),
                     limits = c(0, 0.11)) 
...
Table_mod5h <- emmeans(mod5h, list(pairwise ~ habitat_broad))$`pairwise differences of habitat_broad` %>% 
  as.tibble()
Table_mod5h

Table_mod5h_means <- emmeans(mod5h, list(pairwise ~ habitat_broad))$`emmeans of habitat_broad` %>% 
  as.tibble()
Table_mod4h_means


emmeans_mod5h_habitat <- cld(emmeans(mod5h, list(pairwise ~ habitat_broad)), 
                          Letters = letters) %>% arrange(habitat_broad)
...
alien_data_100 %>%  
  ...
  scale_color_manual(values = col)+
  scale_fill_manual(values = col2)+
  labs(y="Proportion of archaeophytes", x='Grassland type', 
       col="Grassland type",
       fill="Grassland type") +
  # guides(shape = "none") +
  theme_bw()+
  geom_text(data=emmeans_mod5h_habitat,
            aes(x=habitat_broad, y=c(0.3, 0.33, 0.36, 0.07, 0.12, 0.05, 0.05),
                label=emmeans_mod5h_habitat$.group),vjust=0.5, hjust=0.5, 
            size=4, col="black" , position=position_dodge(0))+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) 
