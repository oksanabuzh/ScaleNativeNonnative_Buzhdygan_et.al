...
species <- read_csv("data/database_analysis_summary.csv") |> 
  ...
  mutate(non_native_percent = non_native / total_species,
...
ggplot(species, aes(x = non_native, y = archaeophyte)) +
  ...
  theme_bw()+ scale_x_continuous(breaks = seq(0, 14, by = 2))+
...
species %>% 
  dplyr::select(invasive, neophyte, 
                neophyte_percent, invasive_percent,
                non_native, archaeophyte,
                non_native_percent, archaeophyte_percent) %>% 
cor( method = "pearson") |>
